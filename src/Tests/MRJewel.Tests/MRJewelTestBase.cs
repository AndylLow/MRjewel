...
namespace MRJewel.Tests
{
    public abstract class MRJewelTestBase : AbpIntegratedTestBase<MRJewelTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MRJewelTestBase()
        {
        ...
        protected void UsingDbContext(Action<MRJewelDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MRJewelDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MRJewelDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MRJewelDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MRJewelDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MRJewelDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MRJewelDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MRJewelDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MRJewelDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MRJewelDbContext, Task<T>> func)
        {
            T result;
